...
Feature: Simple Docker Static File Server
  ...
  Scenario: Starting the file server and accessing a file
    ...
    And the file server is started in the directory "www" on port 8001
    When I access the URL "http://localhost:8001/index.html" in the browser
  ...
  Scenario: Accessing a non-existent file
    ...
    And the file server is started in the directory "www" on port 8002
    When I access the URL "http://localhost:8002/non-existent.html" in the browser
    Then I should see a 404 error message

  Scenario: Stopping the file server
    Given the file server is running in the directory "www" on port 8003
    When I stop the file server
    Then the server should no longer be accessible at "http://localhost:8003"